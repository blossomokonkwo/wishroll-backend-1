        ...
        if activity
            ...
            if user.current_device
                ...
                if activity.activity_type == "Comment"
                    ...
                    if post = Post.fetch(activity.content_id)
                        activity_hash = {id: activity.id, phrase: activity.activity_phrase, created_at: activity.created_at, type: activity.activity_type, active_user: {id: activity.active_user.id, username: activity.active_user.username, name: activity.active_user.name, avatar: activity.active_user.avatar_url, verified: activity.active_user.verified}, post: {id: post.id, media_url: post.media_url, thumbnail_url: post.thumbnail_url, caption: post.caption, created_at: post.created_at, updated_at: post.updated_at, user: {id: post.user.id, username: post.user.username, verified: post.user.verified}}}
                    else 
                        return
                    end
