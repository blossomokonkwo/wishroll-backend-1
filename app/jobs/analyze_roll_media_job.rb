class AnalyzeRollMediaJob < ApplicationJob
    ...
    def perform(roll_id, host)
        ...
        roll.media_item.open do |file|
            ...
            roll.thumbnail.attach(io: File.open(thumbnail.path), filename: SecureRandom.uuid + filename)
            roll.thumbnail_url = host + Rails.application.routes.url_helpers.rails_blob_path(roll.thumbnail, only_path: true)
