        ...
        if @comment.save
            if !current_user.posts.include?(@post) #we are checking to ensure that we are not creating an activity object when a user is commenting on their own post                
                ...
                if @comment.original_comment_id
                    ...
                    activity = Activity.new(user_id: user_id, active_user_id: current_user.id, activity_phrase: activity_phrase, activity_type: "Comment", content_id: @comment.id, media_url: @post.media_url)
                    activity.save
            end
