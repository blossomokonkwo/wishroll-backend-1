class Api::V2::Feed::RollsController < APIController
    ...
    def index
        begin
            ...
            @rolls = Roll.includes(:user).where(user: feed_users).and(Roll.where(restricted: false)).order(popularity_rank: :desc, created_at: :desc).offset(offset).limit(limit)

            # check that rolls array isn't empty
            if @rolls.any?
                render :index, status: :ok
            else
                @rolls = Roll.where(restricted: false).order(popularity_rank: :desc, created_at: :desc).offset(offset).limit(limit)
                if @rolls.any? 
                    render :index, status: :ok
                else
