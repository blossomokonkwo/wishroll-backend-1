class Api::V2::Feed::RollsController < APIController
    ...
    def index
        begin
            ...
            @rolls = Roll.where(restricted: false).order(created_at: :desc).offset(offset).limit(limit)  #Roll.includes(:user).where(user: feed_users).order(created_at: :desc).offset(offset).limit(limit)

            # check that rolls array isn't empty
            if @rolls.any?
                render :index, status: :ok
            else
                @rolls = Roll.where(restricted: false).order(created_at: :desc).offset(offset).limit(25)
                if @rolls.any? 
                    render :index, status: :ok
                else
