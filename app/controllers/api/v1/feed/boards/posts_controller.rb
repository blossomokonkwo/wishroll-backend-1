class Api::V1::Feed::Boards::PostsController < APIController
    ...
    def index 
        ...
        @posts = Post.includes(:board).joins(:user).where(user: feed_users).or(Post.where(board: current_user.boards)).and(Post.where.not(user: @current_user.blocked_users.select(:id))).and(Post.where.not(id: @current_user.reported_posts)).and(Post.where.not(user: @current_user.blocker_users)).order(created_at: :desc).offset(offset).limit(limit).to_a
        if @posts.any?
