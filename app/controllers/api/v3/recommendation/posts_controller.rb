class Api::V3::Recommendation::PostsController < ApplicationController
    ...
    def index
        ...
        if content_type = params["content-type"] #if the reccommendation is specified by a content-type, search for all similar posts with the specified content type
            @posts = Post.fetch_multi(Tag.joins(post: {media_item_attachment: :blob}).where("active_storage_blobs.content_type ILIKE ? AND posts.id != ?", "%#{content_type}%", post_id).recommend(document_text).limit(limit).offset(offset).pluck(:post_id)).uniq {|p| p.id}
        else
