class LoginController < ApplicationController
  def create
    ...
    if @user and @user.authenticate(params[:password])
      ...
      payload = {user_id: @user.id, username: @user.username, is_verified: @user.is_verified, email: @user.email, full_name: @user.full_name, profile_picture: @user.profile_picture.attached? ? url_for(@user.profile_picture) : nil}
