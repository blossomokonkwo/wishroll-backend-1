class AppearancesChannel < ApplicationCable::Channel
  ...
  def appearance(data)
    ...
    chat_room_user = ChatRoomUser.find_by(user_id: current_user.id, chat_room_id: data[:chat_room_id])
  ...
  def typing(data)
    ...
    typing_user = User.find(data[:typing_user_id])
    chat_room = ChatRoom.find(data[:chat_room_id])
    if typing_user and chat_room
      AppearancesChannel.broadcast_to(chat_room, typing_user.username, {typing: true})
    end
  ...
  def end_typing(data)
    ...
    end_typing_user = User.find(data[:end_typing_user_id])
    chat_room = ChatRoom.find(data[:chat_room_id])
